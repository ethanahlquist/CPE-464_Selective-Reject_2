// Hugh Smith April 20173 // Network code to support TCP/UDP client and server connections4 // June 2018 - modified to support IPv6
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
#include <sys/stat.h>
...
// only use if the CPE464 sendtoErr library exists
#if __has_include("cpe464.h")
#include "cpe464.h"
#endif
...
int safeSendto(uint8_t  * packet, uint32_t len, Connection * to)
{
    ...
    if((send_len = sendto(to->sk_num, packet, len, 0,
...
int udpServerSetup(int portNumber){
    ...
    fprintf(fp, "%d", ntohs(server.sin6_port));
    fclose(fp);
...
int select_call(int32_t  socket_num, int32_t seconds, int32_t microseconds)
{
    ...
    // since timeout defaults to pointing to NULL
